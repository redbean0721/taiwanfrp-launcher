...
func (ctl *Control) HandleNewProxyResp(inMsg *msg.NewProxyResp) {
	...
	if err != nil {
		...
		// 顯示代理的域名和端口
		xl.Info("[%s] 啟動成功，連線位置: %s%s", inMsg.ProxyName, ctl.clientCfg.ServerAddr, inMsg.RemoteAddr)
	}
}
		...
		case <-hbSendCh:
			...
			case <-hbCheckCh:
				if time.Since(ctl.lastPong) > time.Duration(ctl.clientCfg.HeartbeatTimeout)*time.Second {
					// 不因為心跳逾時主動斷線，僅在伺服器明確回傳錯誤時再關閉連線。
					xl.Warn("heart超時，等待伺服器明確回應")
					// 避免在伺服器無回應時每秒重複刷屏。
					ctl.lastPong = time.Now()
				}
