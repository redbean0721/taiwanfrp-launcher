...
func (svr *Service) Run(ctx context.Context) error {
	// 讀取啟動器登入資訊並顯示當前使用者
	username := ""
	for _, infoPath := range []string{"lib/info.json", "info.json"} {
	...
	log.Infof("當前使用者: %s  若要重置客戶端 請刪除lib文件夾", username)
	log.Infof("歡迎使用taiwanfrp客戶端，請稍等...")
	ctx, cancel := context.WithCancelCause(ctx)
	svr.ctx = xlog.NewContext(ctx, xlog.FromContextSafe(ctx))
	svr.cancel = cancel

	// 設定自訂 DNS 伺服器，若設定檔沒有則使用啟動器傳入的環境變數
	dnsServer := strings.TrimSpace(svr.common.DNSServer)
	if dnsServer == "" {
		dnsServer = strings.TrimSpace(os.Getenv("TAIWANFRP_DNS_SERVER"))
	}
	if dnsServer != "" {
		netpkg.SetDefaultDNSAddress(dnsServer)
	}

	if svr.vnetController != nil {
		if err := svr.vnetController.Init(); err != nil {
			log.Errorf("初始化虛擬網路控制器失敗: %v", err)
			return err
		}
		go func() {
			log.Infof("虛擬網路控制器已啟動...")
			if err := svr.vnetController.Run(); err != nil {
				log.Warnf("虛擬網路控制器異常結束: %v", err)
			}
		}()
	}
	...
	if svr.webServer != nil {
		go func() {
			log.Infof("管理伺服器監聽於 %s", svr.webServer.Address())
			if err := svr.webServer.Run(); err != nil {
				log.Warnf("管理伺服器異常結束: %v", err)
			}
		}()
	}
	...
	if svr.ctl == nil {
		...
		return fmt.Errorf("登入伺服器失敗: %v。已啟用 loginFailExit，不會再重試", cancelCause.Err)
	}
...
func (svr *Service) keepControllerWorking() {
	...
	// 有一種情況是登入成功但因某些原因控制連線立即結束，這時需要限制重連頻率。
	// 前三次重試間隔很短，之後指數增加，最大間隔 20 秒。
	wait.BackoffUntil(func() (bool, error) {
		// loopLoginUntilSuccess 會持續嘗試登入伺服器直到成功。
		svr.loopLoginUntilSuccess(20*time.Second, false)
		if svr.ctl != nil {
			<-svr.ctl.Done()
			return false, errors.New("控制連線已關閉，重新嘗試登入")
		}
		// 若 ctl 為 nil，表示登入失敗且服務已關閉。
		return false, nil
...
func (svr *Service) login() (conn net.Conn, connector Connector, err error) {
	...
	xl.Infof("成功登入伺服器，取得 run id [%s]", loginRespMsg.RunID)
	return
}
