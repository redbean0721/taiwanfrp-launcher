...
func (sv *SUDPVisitor) dispatcher() {
	...
	for {
		...
		if err != nil {
			xl.Warnf("newVisitorConn 連線 frps 失敗: %v，嘗試重新連線", err)
			continue
		}
	...
	workConnReaderFn := func(conn net.Conn) {
		...
		for {
			...
			if rawMsg, errRet = msg.ReadMsg(conn); errRet != nil {
				xl.Warnf("從 workconn 讀取使用者 udp 連線錯誤: %v", errRet)
				return
			}
			...
			case *msg.Ping:
				xl.Debugf("frpc 訪客收到 frpc 的 ping 訊息")
				continue
			case *msg.UDPPacket:
				if errRet := errors.PanicToError(func() {
					sv.readCh <- m
					xl.Tracef("frpc 訪客從 workConn 收到 udp 封包: %s", m.Content)
				}); errRet != nil {
					xl.Infof("udp work 連線的 reader goroutine 已關閉")
					return
				}
			}
		}
	...
	workConnSenderFn := func(conn net.Conn) {
		...
		if firstPacket != nil {
			if errRet = msg.WriteMsg(conn, firstPacket); errRet != nil {
				xl.Warnf("udp work 連線的 sender goroutine 已關閉: %v", errRet)
				return
			}
			xl.Tracef("傳送 udp 封包到 workConn: %s", firstPacket.Content)
		}
		...
		for {
			...
			case udpMsg, ok := <-sv.sendCh:
				if !ok {
					xl.Infof("udp work 連線的 sender goroutine 已關閉")
					return
				}
				...
				xl.Tracef("傳送 udp 封包到 workConn: %s", udpMsg.Content)
			case <-closeCh:
				return
			}
		}
