...
func (svr *Service) apiReload(w http.ResponseWriter, r *http.Request) {
	...
	log.Infof("API 請求 [/api/reload]")
	defer func() {
		log.Infof("API 回應 [/api/reload]，狀態碼 [%d]", res.Code)
	...
	if err != nil {
		res.Code = 400
		res.Msg = err.Error()
		log.Warnf("重新載入 frpc 代理設定檔錯誤: %s", res.Msg)
		return
	}
...
func (svr *Service) apiStop(w http.ResponseWriter, _ *http.Request) {
	...
	log.Infof("API 請求 [/api/stop]")
	defer func() {
		log.Infof("API 回應 [/api/stop]，狀態碼 [%d]", res.Code)
		w.WriteHeader(res.Code)
		if len(res.Msg) > 0 {
			_, _ = w.Write([]byte(res.Msg))
		}
...
func (svr *Service) apiStatus(w http.ResponseWriter, _ *http.Request) {
	...
	log.Infof("HTTP 請求 [/api/status]")
	defer func() {
		log.Infof("HTTP 回應 [/api/status]")
...
func (svr *Service) apiGetConfig(w http.ResponseWriter, _ *http.Request) {
	...
	log.Infof("HTTP 取得請求 [/api/config]")
	defer func() {
		log.Infof("HTTP 取得回應 [/api/config]，狀態碼 [%d]", res.Code)
		w.WriteHeader(res.Code)
		if len(res.Msg) > 0 {
			_, _ = w.Write([]byte(res.Msg))
		}
	...
	if err != nil {
		res.Code = 400
		res.Msg = err.Error()
		log.Warnf("讀取 frpc 設定檔錯誤: %s", res.Msg)
		return
	}
...
func (svr *Service) apiPutConfig(w http.ResponseWriter, r *http.Request) {
	...
	log.Infof("HTTP 更新請求 [/api/config]")
	defer func() {
		log.Infof("HTTP 更新回應 [/api/config]，狀態碼 [%d]", res.Code)
		w.WriteHeader(res.Code)
		if len(res.Msg) > 0 {
			_, _ = w.Write([]byte(res.Msg))
		}
	...
	if err != nil {
		res.Code = 400
		res.Msg = fmt.Sprintf("讀取請求內容錯誤: %v", err)
		log.Warnf("%s", res.Msg)
		return
	}
	...
	if err := os.WriteFile(svr.configFilePath, body, 0o600); err != nil {
		res.Code = 500
		res.Msg = fmt.Sprintf("寫入 frpc 設定檔內容錯誤: %v", err)
		log.Warnf("%s", res.Msg)
		return
	}
}
