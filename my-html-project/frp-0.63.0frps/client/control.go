...
func (ctl *Control) handleReqWorkConn(_ msg.Message) {
	...
	if err = ctl.sessionCtx.AuthSetter.SetNewWorkConn(m); err != nil {
		xl.Warnf("NewWorkConn 認證時發生錯誤: %v", err)
		workConn.Close()
		return
	}
	if err = msg.WriteMsg(workConn, m); err != nil {
		xl.Warnf("work connection 寫入伺服器失敗: %v", err)
		workConn.Close()
		return
	}
	...
	if err = msg.ReadMsgInto(workConn, &startMsg); err != nil {
		xl.Tracef("work connection 在回應 StartWorkConn 訊息前已關閉: %v", err)
		workConn.Close()
		return
	}
	if startMsg.Error != "" {
		xl.Errorf("StartWorkConn 包含錯誤: %s", startMsg.Error)
		workConn.Close()
		return
	}
...
func (ctl *Control) handleNewProxyResp(m msg.Message) {
	...
	if err != nil {
		xl.Warnf("[%s] 啟動失敗: %v", inMsg.ProxyName, err)
	} else {
		serverAddr := ctl.sessionCtx.Common.ServerAddr
		remoteAddr := inMsg.RemoteAddr
		xl.Infof("[%s] 啟動代理成功，連接位置: %s%s", inMsg.ProxyName, serverAddr, remoteAddr)
	}
}
...
func (ctl *Control) handleNatHoleResp(m msg.Message) {
	...
	if !ok {
		xl.Tracef("將 NatHoleResp 訊息分派給相關代理時發生錯誤")
	}
}
...
func (ctl *Control) handlePong(m msg.Message) {
	...
	if inMsg.Error != "" {
		xl.Errorf("pong 訊息包含錯誤: %s", inMsg.Error)
		ctl.closeSession()
		return
	}
	ctl.lastPong.Store(time.Now())
...
func (ctl *Control) heartbeatWorker() {
	...
	if ctl.sessionCtx.Common.Transport.HeartbeatInterval > 0 {
		...
		sendHeartBeat := func() (bool, error) {
			xl.Debugf("傳送心跳訊號到伺服器")
			pingMsg := &msg.Ping{}
			if err := ctl.sessionCtx.AuthSetter.SetPing(pingMsg); err != nil {
				xl.Warnf("Ping 認證時發生錯誤: %v，略過此次 Ping 訊息", err)
				return false, err
			}
	...
	if ctl.sessionCtx.Common.Transport.HeartbeatInterval > 0 && ctl.sessionCtx.Common.Transport.HeartbeatTimeout > 0 {
		go wait.Until(func() {
			if time.Since(ctl.lastPong.Load().(time.Time)) > time.Duration(ctl.sessionCtx.Common.Transport.HeartbeatTimeout)*time.Second {
				// 不因為心跳逾時主動斷線，僅在伺服器明確回傳錯誤時關閉連線。
				xl.Warnf("心跳逾時，等待伺服器明確回應")
				// 避免在伺服器無回應時每秒重複刷屏。
				ctl.lastPong.Store(time.Now())
				return
			}
