...
func (sv *STCPVisitor) handleConn(userConn net.Conn) {
	...
	if err != nil {
		xl.Warnf("傳送 newVisitorConnMsg 給伺服器失敗: %v", err)
		return
	}
	...
	if err != nil {
		xl.Warnf("取得 newVisitorConnRespMsg 失敗: %v", err)
		return
	}
	_ = visitorConn.SetReadDeadline(time.Time{})

	if newVisitorConnRespMsg.Error != "" {
		xl.Warnf("啟動新訪客連線失敗: %s", newVisitorConnRespMsg.Error)
		return
	}
